...
dependencies = [
    ...
    ['core', 'liberty', 'libertycampaign', 'libertystory', 'left2die'],
...
def write_to_file(keylist, conflictsset, duplicateset):
    ...
    with open('output/ConflictChecks.py', 'w') as f:
...
def generate_checks():
    ...
    for index in range(len(dependencies)):
        ...
        if 'CampaignVehicles' in temp_keydict:
            button_tree.write(str(index)+' button '+path_destination_name+'.xml')
            unit_tree.write(str(index)+' unit '+path_destination_name+'.xml')
